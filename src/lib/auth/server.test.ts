...
const { etatDb } = vi.hoisted(() => ({ etatDb: { rows: [] as Array<{ passwordHash: string }> } }));
vi.mock("@/db", () => ({
  db: {
...
async function jetonPour(passwordHash: string): Promise<string> {
  ...
  return signerSession({ userId: 1, email: "a@yvia.io", exp: Date.now() + DUREE_SESSION_MS, pv });
}
...
describe("getSession : révocation par changement de mot de passe", () => {
  it("renvoie la session quand le pv correspond au hash courant", async () => {
    ...
    etatDb.rows = [{ passwordHash: hash }];
    expect((await getSession())?.userId).toBe(1);
  });
  ...
  it("révoque la session quand le mot de passe a changé (pv obsolète)", async () => {
    ...
    etatDb.rows = [{ passwordHash: "nouveau-hash" }];
